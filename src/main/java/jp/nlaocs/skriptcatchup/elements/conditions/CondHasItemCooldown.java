...
public class CondHasItemCooldown extends Condition {
    ...
    public boolean check(Event event) {
        return players.check(event, (player) ->
                itemtypes.check(event, (itemType) ->
                        !itemType.getTypes().isEmpty() && player.hasCooldown(itemType.getMaterial())
                )
        );
    }
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return PropertyCondition.toString(this, PropertyCondition.PropertyType.HAVE, event, debug, players,
